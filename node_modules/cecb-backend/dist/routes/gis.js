...
router.get('/check', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { lat, lng } = coordSchema.parse(req.query);
    const flags = [];
    for (const [layerType, threshold] of Object.entries(THRESHOLDS)) {
        const features = DEMO_LAYERS[layerType];
...
// ─── GET /api/gis/layers ──────────────────────────────────────────────────────
router.get('/layers', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (_req, res) => {
    res.json({
        success: true,
        data: {
...
router.post('/save-flags', auth_1.authenticate, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { applicationId, flags } = zod_1.z.object({
        applicationId: zod_1.z.string().uuid(),
        flags: zod_1.z.array(zod_1.z.object({
            flagType: zod_1.z.string(),
            layerName: zod_1.z.string(),
            distanceM: zod_1.z.number(),
            severity: zod_1.z.string().default('HIGH'),
        })),
    }).parse(req.body);
    // Delete existing flags for this application
    await prisma_1.prisma.gisRiskFlag.deleteMany({ where: { applicationId } });
    const savedFlags = await prisma_1.prisma.gisRiskFlag.createMany({
